      ...
      node(label) {
         ...
         container('build-container'){
              ...
              if (namespace) {
                  ...
                  s3Download(file:'.env', bucket:'env.faldax', path:"ethereum-node-js/${namespace}/.env", force:true)
                }
                sh "ls -a"
                sh "docker build -t ${imageRepo}/ethereum:${imageTag}  ."
                sh "docker push  ${imageRepo}/ethereum:${imageTag}"
                sh "helm upgrade --install --namespace ${namespace} --set image.tag=${imageTag} ${namespace}-ethereum-backend -f chart/values-${namespace}.yaml chart/"
              }
         }
         }
